...
import (
	...
	"github.com/khulnasoft/kengine/kengine_utils/telemetry"
	"github.com/khulnasoft/kengine/kengine_utils/utils"
...
var (
	FileServerExternal       = utils.GetEnvOrDefault("KHULNASOFT_FILE_SERVER_EXTERNAL", "false") == "true"
	FileServerProtocol       string
	FileServerSecure         = utils.GetEnvOrDefault("KHULNASOFT_FILE_SERVER_SECURE", "false") == "true"
	FileServerHost           = utils.GetEnvOrDefault("KHULNASOFT_FILE_SERVER_HOST", "khulnasoft-file-server")
	FileServerPort           = utils.GetEnvOrDefault("KHULNASOFT_FILE_SERVER_PORT", "9000")
	FileServerRegion         = utils.GetEnvOrDefault("KHULNASOFT_FILE_SERVER_REGION", "")
	FileServerBucket         = utils.GetEnvOrDefault("KHULNASOFT_FILE_SERVER_BUCKET", string(NonSaaSDirKey))
	FileServerDatabaseBucket = utils.GetEnvOrDefault("KHULNASOFT_FILE_SERVER_DB_BUCKET", string(DatabaseDirKey))
...
func (mfm *FileServerFileManager) ExposeFile(ctx context.Context, filePath string, addFilePathPrefix bool, expires time.Duration, reqParams url.Values, consoleURL string) (string, error) {
	...
	if !FileServerExternal {
		...
		// Format: khulnasoft.customer.com:8080 or 56.56.56.56
		if consoleURL == "" {
