...
func runSystemScheduledTasks(ctx context.Context, messagePayload map[string]interface{}) error {
	...
	khulnasoftSystemScan := false
	if _, ok := payload["khulnasoft_system_scan"]; ok {
		khulnasoftSystemScan = payload["khulnasoft_system_scan"].(bool)
	}
	...
	scanTrigger := model.ScanTriggerCommon{NodeIDs: nodeIds,
		Filters: model.ScanFilter{}, IsPriority: isPriority, KhulnasoftSystemScan: khulnasoftSystemScan}
...
func runCustomScheduledTasks(ctx context.Context, messagePayload map[string]interface{}) error {
	...
	scanTrigger := model.ScanTriggerCommon{NodeIDs: nodeIds,
		Filters: scanFilter, IsPriority: payload.IsPriority, KhulnasoftSystemScan: payload.KhulnasoftSystemScan}
