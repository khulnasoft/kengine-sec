...
export const scanSecretApiAction = async ({
  ...
  const scanKhulnasoftSystem = formData.get('scanKhulnasoftSystem') === 'on';
  ...
  const requestBody: ModelSecretScanTriggerReq = {
    ...
    khulnasoft_system_scan: scanKhulnasoftSystem,
  };
...
const ScanKhulnasoftSystem = () => {
  return (
    <div className="mt-6 flex items-center gap-x-1">
      <Switch
        label="Scan selected Khulnasoft images/containers"
        name="scanKhulnasoftSystem"
      />
      <Tooltip
        content="If the resources you have selected for the scan include Khulnasoft images/containers, you can enable this option to scan them. Khulnasoft images/containers are not scanned by default."
        triggerAsChild
      >
...
export const SecretScanConfigureForm = ({
  ...
  return (
    ...
    >
      ...
      {wantKhulnasoftSystem(data) ? <ScanKhulnasoftSystem /> : null}
