...
const defaultNamespace = 'khulnasoft';
...
const InformationForm = ({
  ...
  useMemo(() => {
    ...
    const installCommand = `helm install khulnasoft-agent khulnasoft/khulnasoft-agent \\
  --set managementConsoleUrl=${window.location.host ?? '---CONSOLE-IP---'} \\
  --set khulnasoftKey=${dfApiKey} \\
...
export const K8ConnectorForm = () => {
  const [instruction, setInstruction] =
    useState(`helm repo add khulnasoft https://khulnasoft-helm-charts.s3.amazonaws.com/kengine
helm repo update

helm install khulnasoft-agent khulnasoft/khulnasoft-agent \\
  --set managementConsoleUrl=${window.location.host ?? '---CONSOLE-IP---'} \\
  --set khulnasoftKey=${PLACEHOLDER_API_KEY} \\
  --set global.imageTag=${''} \\
      ...
      <Stepper>
        ...
        >
          <div className="text-p7a text-text-text-and-icon">
            Deploy Khulnasoft agent Kubernetes Scanner. Find out more information by{' '}
            <DFLink
              href={`https://community.khulnasoft.com/kengine/docs/v2.2/sensors/kubernetes`}
              target="_blank"
              rel="noreferrer"
              className="mt-2"
            >
