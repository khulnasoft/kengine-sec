...
func csvs2FieldsFilter(csvs []string) (khulnasoft_server_client.ReportersFieldsFilters, error) {
	println(len(csvs))
	if len(csvs) == 0 {
		return khulnasoft_server_client.ReportersFieldsFilters{}, nil
	}
	...
	for i := range csvs {
		...
		if len(key_value) != 2 {
			return khulnasoft_server_client.ReportersFieldsFilters{}, fmt.Errorf("Unexpected entry format: %v", csvs[i])
		}
		filters[key_value[0]] = append(filters[key_value[0]], key_value[1])
	}
	return khulnasoft_server_client.ReportersFieldsFilters{
...
var graphTopologySubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		var field_filters khulnasoft_server_client.ReportersFieldsFilters
		...
		filters := khulnasoft_server_client.GraphTopologyFilters{
		...
		var res *khulnasoft_server_client.ModelGraphResult
...
var graphThreatSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		req = req.GraphThreatFilters(khulnasoft_server_client.GraphThreatFilters{
			Type:              issue_filter,
...
var attackPathsSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		req = req.GraphIndividualThreatGraphRequest(
			khulnasoft_server_client.GraphIndividualThreatGraphRequest{
