...
launch_discovery() {
  echo "Starting khulnasoft-agent discovery... with hostname $SCOPE_HOSTNAME"
  bash -x $KE_INSTALL_DIR/home/khulnasoft/run_discovery_loop.sh &
}
...
mkdir -p $KE_INSTALL_DIR/var/log/fenced/
chmod +x $KE_INSTALL_DIR/home/khulnasoft/*.sh

echo "Start Khulnasoft services... Console is $MGMT_CONSOLE_URL"
launch_package_scanner

echo "Starting discovery logs..." >>$KE_INSTALL_DIR/var/log/fenced/discovery.logfile
launch_discovery
...
tail --follow=name $KE_INSTALL_DIR/var/log/fenced/discovery.logfile | sed 's/^/DF_DISCOVERY: /g' &
