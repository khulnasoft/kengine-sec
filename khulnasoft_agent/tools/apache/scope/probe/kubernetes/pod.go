...
func (p *pod) GetNode() report.TopologyNode {
	...
	if appLabel, found := p.Labels()[report.KhulnasoftK8sAppLabelKey]; found {
		if appLabel == report.KhulnasoftK8sConsoleAppLabelValue || appLabel == report.KhulnasoftK8sAgentAppLabelValue {
			metadata.IsKhulnasoftSystem = true
		}
	}
