	...
	while getopts "h:u:p:k:m:a:o:s:" option; do
		case $option in 
			...
			p) mgmt_console_port=$OPTARG
				...
				;;
			k) khulnasoft_key=$OPTARG
				echo "key:"$khulnasoft_key
...
launch_khulnasoftd() {
  ...
  until [[ "$(ls $KE_INSTALL_DIR/var/log/supervisor/khulnasoftd* 2>/dev/null | wc -w)" != "0" ]]; do
    sleep 5
  done
  tail -f $KE_INSTALL_DIR/var/log/supervisor/khulnasoftd*
}
