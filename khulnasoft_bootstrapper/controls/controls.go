...
import (
	...
	linuxScanner "github.com/khulnasoft-lab/compliance/scanner"
	linuxScannerUtil "github.com/khulnasoft-lab/compliance/util"
...
func SetClusterAgentControls(k8sClusterName string) {
	...
	_, err = exec.Command("/bin/sh", dfUtils.GetDfInstallDir()+"/home/khulnasoft/token.sh").CombinedOutput()
	if err != nil {
		...
		func(req ctl.SendAgentDiagnosticLogsRequest) error {
			...
			return SendAgentDiagnosticLogs(req,
				[]string{dfUtils.GetDfInstallDir() + "/var/log/supervisor",
					...
					dfUtils.GetDfInstallDir() + "/var/log/khulnasoftd"},
				[]string{})
		})
	if err != nil {
		log.Error().Err(err).Msg("set controls")
