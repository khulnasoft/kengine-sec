...
if [ -z "$KHULNASOFT_NEO4J_USER" ]; then
    export KHULNASOFT_NEO4J_USER="neo4j"
fi
if [ -z "$KHULNASOFT_NEO4J_PASSWORD" ]; then
    export KHULNASOFT_NEO4J_PASSWORD="e16908ffa5b9f8e9d4ed"
fi
if [ -z "$NEO4J_AUTH" ]; then
    export NEO4J_AUTH=$KHULNASOFT_NEO4J_USER/$KHULNASOFT_NEO4J_PASSWORD
fi
...
backup_db() {
    ...
    if [ -n "$KE_REMOTE_BACKUP_ROOT" ]
    then
        rclone sync /backups :$PROVIDER:$KE_REMOTE_BACKUP_ROOT --include "*.dump"
    fi
    trap backup_db USR1
}
...
if [ -n "$KE_REMOTE_BACKUP_ROOT" ]
then
    rclone sync :$PROVIDER:$KE_REMOTE_BACKUP_ROOT /backups
fi
