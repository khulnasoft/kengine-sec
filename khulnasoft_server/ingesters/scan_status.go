...
type KhulnasoftSystemScanError struct {
	NodeID string
}

func (ve *KhulnasoftSystemScanError) Error() string {
	return fmt.Sprintf("khulnasoft_system_scan=true required to scan Khulnasoft image/pod/container: %s", ve.NodeID)
}
...
func AddNewScan(ctx context.Context,
	...
	khulnasoftSystemScan bool) error {
	...
	res, err := tx.Run(ctx, fmt.Sprintf(`
		...
		RETURN n IS NOT NULL AS Exists, n.is_khulnasoft_system`,
	...
	if !khulnasoftSystemScan && rec.Values[1] != nil && rec.Values[1].(bool) {
		return &KhulnasoftSystemScanError{
			NodeID: nodeID,
		}
	}
	...
	case controls.Image:
		...
		ORDER BY rand()
		LIMIT 1
		MATCH (l:Node{node_id: "khulnasoft-console-cron"})
