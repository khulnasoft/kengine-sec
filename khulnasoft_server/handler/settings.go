...
import (
	...
	api_messages "github.com/khulnasoft/kengine/khulnasoft_server/constants/api-messages"
	"github.com/khulnasoft/kengine/khulnasoft_server/model"
	"github.com/khulnasoft/kengine/khulnasoft_server/pkg/constants"
	"github.com/khulnasoft/kengine/khulnasoft_server/pkg/sendemail"
...
func (h *Handler) TestConfiguredEmail(w http.ResponseWriter, r *http.Request) {
	...
	err = emailSender.Send([]string{user.Email}, "Khulnasoft Testmail", "This is a test email", "", nil)
	if err != nil {
		h.respondError(&InternalServerError{err}, w)
		return
	}
...
func (h *Handler) TestUnconfiguredEmail(w http.ResponseWriter, r *http.Request) {
	...
	// send email to user
	email := user.Email
	err = emailSender.Send([]string{email}, "Khulnasoft Testmail", "This is a test email", "", nil)
	if err != nil {
...
const (
	startAgentScript     = "start_khulnasoft_agent.sh"
	uninstallAgentScript = "uninstall_khulnasoft_agent.sh"
	agentBinaryFileAmd64 = "khulnasoft-agent-amd64-%s.tar.gz"
	agentBinaryFileArm64 = "khulnasoft-agent-arm64-%s.tar.gz"
)
