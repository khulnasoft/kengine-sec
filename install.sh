...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        echo "  -t, --image-tag      Specify the image tag for the Khulnasoft components (default: $DEFAULT_IMAGE_TAG) (optional)"
...
kubernetes_deployment() {
    ...
    CONSOLE_VALUES_FILE="deployment-scripts/helm-charts/khulnasoft-console/values.yaml"
    ROUTER_VALUES_FILE="deployment-scripts/helm-charts/khulnasoft-router/values.yaml"
    ...
    # Install khulnasoft-router
    helm upgrade --install khulnasoft-router deployment-scripts/helm-charts/khulnasoft-router --set global.imageTag="$IMAGE_TAG" --set global.storageClass="$STORAGE_CLASS" -f $ROUTER_VALUES_FILE

    # Install khulnasoft-console
    helm upgrade --install khulnasoft-console deployment-scripts/helm-charts/khulnasoft-console --set global.imageTag="$IMAGE_TAG" --set global.storageClass="$STORAGE_CLASS" --set neo4j.secrets.NEO4J_AUTH=neo4j/$NEO4J_PASSWORD -f $CONSOLE_VALUES_FILE
