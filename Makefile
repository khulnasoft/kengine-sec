...
export IMAGE_REPOSITORY?=quay.io/khulnasoft
...
alpine_builder:
	docker build --tag=$(IMAGE_REPOSITORY)/khulnasoft_builder_ce:$(DF_IMG_TAG) -f docker_builders/Dockerfile-alpine .

.PHONY: debian_builder
debian_builder:
	docker build --build-arg DF_IMG_TAG=${DF_IMG_TAG} --build-arg IMAGE_REPOSITORY=${IMAGE_REPOSITORY} --tag=$(IMAGE_REPOSITORY)/khulnasoft_glibc_builder_ce:$(DF_IMG_TAG) -f docker_builders/Dockerfile-debian .
...
agent-binary-tar:
	...
	ID=$$(docker create $(IMAGE_REPOSITORY)/khulnasoft_agent_ce:$(DF_IMG_TAG)); \
...
.PHONY: redis
redis:
	(cd khulnasoft_redis && docker build --tag=$(IMAGE_REPOSITORY)/khulnasoft_redis_ce:$(DF_IMG_TAG) .)

.PHONY: postgres
postgres:
	docker build --tag=$(IMAGE_REPOSITORY)/khulnasoft_postgres_ce:$(DF_IMG_TAG) -f khulnasoft_postgres/Dockerfile ./khulnasoft_postgres

.PHONY: kafka-broker
kafka-broker:
	docker build -t $(IMAGE_REPOSITORY)/khulnasoft_kafka_broker_ce:$(DF_IMG_TAG) -f ./khulnasoft_kafka/kafka-broker-Dockerfile ./khulnasoft_kafka

.PHONY: router
router:
	docker build --build-arg is_dev_build=$(IS_DEV_BUILD) -t $(IMAGE_REPOSITORY)/khulnasoft_router_ce:$(DF_IMG_TAG) $(KHULNASOFT_ROUTER_DIR)

.PHONY: file-server
file-server:
	docker build -t $(IMAGE_REPOSITORY)/khulnasoft_file_server_ce:$(DF_IMG_TAG) $(KHULNASOFT_FILE_SERVER_DIR)
...
.PHONY: jaeger
jaeger:
	docker build -t $(IMAGE_REPOSITORY)/khulnasoft_telemetry_ce:$(DF_IMG_TAG) $(KHULNASOFT_TELEMETRY_DIR)

.PHONY: graphdb
graphdb:
	docker build -f ./khulnasoft_neo4j/Dockerfile --build-arg IMAGE_REPOSITORY=$(IMAGE_REPOSITORY) --build-arg DF_IMG_TAG=$(DF_IMG_TAG) -t $(IMAGE_REPOSITORY)/khulnasoft_neo4j_ce:$(DF_IMG_TAG) ./khulnasoft_neo4j
...
ui:
	...
	docker build -f $(KHULNASOFT_FRONTEND_DIR)/Dockerfile -t $(IMAGE_REPOSITORY)/khulnasoft_ui_ce:$(DF_IMG_TAG) $(KHULNASOFT_FRONTEND_DIR) && \
...
secretscanner: bootstrap-agent-plugins
	docker build --tag=$(IMAGE_REPOSITORY)/khulnasoft_secret_scanner_ce:$(DF_IMG_TAG) -f $(SECRET_SCANNER_DIR)/Dockerfile $(SECRET_SCANNER_DIR)

.PHONY: malwarescanner
malwarescanner: bootstrap-agent-plugins
	docker build --tag=$(IMAGE_REPOSITORY)/khulnasoft_malware_scanner_ce:$(DF_IMG_TAG) -f $(MALWARE_SCANNER_DIR)/Dockerfile $(MALWARE_SCANNER_DIR)

.PHONY: packagescanner
packagescanner: bootstrap-agent-plugins
	docker build --tag=$(IMAGE_REPOSITORY)/khulnasoft_package_scanner_ce:$(DF_IMG_TAG) -f $(PACKAGE_SCANNER_DIR)/Dockerfile $(PACKAGE_SCANNER_DIR)

.PHONY: compliancescanner
compliancescanner:
	docker build --tag=$(IMAGE_REPOSITORY)/khulnasoft_compliance_scanner_ce:$(DF_IMG_TAG) -f $(COMPLIANCE_SCANNER_DIR)/Dockerfile $(COMPLIANCE_SCANNER_DIR)
...
openapi: server
	...
	-v $(PWD):/app $(IMAGE_REPOSITORY)/khulnasoft_server_ce:$(DF_IMG_TAG) \
...
.PHONY: publish-redis
publish-redis:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_redis_ce:$(DF_IMG_TAG)

.PHONY: publish-postgres
publish-postgres:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_postgres_ce:$(DF_IMG_TAG)

.PHONY: publish-kafka
publish-kafka:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_kafka_broker_ce:$(DF_IMG_TAG)

.PHONY: publish-router
publish-router:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_router_ce:$(DF_IMG_TAG)

.PHONY: publish-file-server
publish-file-server:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_file_server_ce:$(DF_IMG_TAG)

.PHONY: publish-server
publish-server:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_server_ce:$(DF_IMG_TAG)

.PHONY: publish-worker
publish-worker:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_worker_ce:$(DF_IMG_TAG)

.PHONY: publish-ui
publish-ui:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_ui_ce:$(DF_IMG_TAG)

.PHONY: publish-agent
publish-agent:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_agent_ce:$(DF_IMG_TAG)

.PHONY: publish-cluster-agent
publish-cluster-agent:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_cluster_agent_ce:$(DF_IMG_TAG)

.PHONY: publish-packagescanner
publish-packagescanner:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_package_scanner_ce:$(DF_IMG_TAG)

.PHONY: publish-secretscanner
publish-secretscanner:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_secret_scanner_ce:$(DF_IMG_TAG)

.PHONY: publish-malwarescanner
publish-malwarescanner:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_malware_scanner_ce:$(DF_IMG_TAG)

.PHONY: publish-graphdb
publish-graphdb:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_neo4j_ce:$(DF_IMG_TAG)

.PHONY: publish-jaeger
publish-jaeger:
	docker push $(IMAGE_REPOSITORY)/khulnasoft_telemetry_ce:$(DF_IMG_TAG)
